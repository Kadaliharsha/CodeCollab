...
def seed_database():
    """ This function populates the database with initial coding problems and their test cases.
    """
    with app.app_context():
        # Ensure all tables exist before attempting to delete/insert
        db.create_all()
        
        # Clean up old data
        # This prevents creating duplicate problems every time we run the script
        ...
        # We need to commit here so that problem1 gets an ID from the database
        db.session.commit()
        ...
        tc1_3 = TestCase(problem_id=problem1.id, input_data='""', expected_output='""') # Edge case
        ...
        # Commit again to get the ID for problem2
        db.session.commit()
